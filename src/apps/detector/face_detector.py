...
face_cascade = cv2.CascadeClassifier(
    os.path.join(CLASSIFIERS_ROOT, 'haarcascade_frontalface_default.xml'))
eye_cascade = cv2.CascadeClassifier(
    os.path.join(CLASSIFIERS_ROOT, 'haarcascade_eye.xml'))
nose_cascade = cv2.CascadeClassifier(
    os.path.join(CLASSIFIERS_ROOT, 'haarcascade_mcs_nose.xml'))
mouth_cascade = cv2.CascadeClassifier(
    os.path.join(CLASSIFIERS_ROOT, 'haarcascade_mcs_mouth.xml'))
...
def detect_coordinates(image_path):
    ...
    for (p, q, r, s) in faces:
        faces_coordinates.append([p, q, r, s])
